<template>
  <div>
    <section class="facts-figures-sec-main-div common-padding-div">
      <div class="container">
        <div class="row" id="counter">
          <div class="col-md-4">
            <div class="counter-box-div">
              <div class="facts-img-div">
                <img src="image/years-in-business-icon.png" class="img-fluid" />
              </div>
              <h6>Years In Business</h6>
              <p><span class="counter-value" data-count="6">6</span></p>
            </div>
          </div>
          <div class="col-md-4">
            <div class="counter-box-div">
              <div class="facts-img-div">
                <img
                  src="image/successfully-project-icon.png"
                  class="img-fluid"
                />
              </div>
              <h6>Projects Successfully Delivered</h6>
              <p><span class="counter-value" data-count="97">97</span></p>
            </div>
          </div>
          <div class="col-md-4">
            <div class="counter-box-div">
              <div class="facts-img-div">
                <img src="image/happy-clien-icon.png" class="img-fluid" />
              </div>
              <h6>Satisfied Clients</h6>
              <p><span class="counter-value" data-count="40">40</span></p>
            </div>
          </div>
        </div>
      </div>
    </section>
  </div>
</template>

<script>
export default {
  name: 'SatisfiedClients',
}
</script>

